...
export default function WatchList() {
  ...
  const handleRemove = async (e: React.MouseEvent, itemId: string) => {
    ...
    try {
      ...
      handleFirestoreError(error, OperationType.DELETE, `users/${user.uid}/watchlist/${itemId}`);
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        {loading ? (
              ...
              <motion.div
                ...
                onClick={() => {
                  if (item.mediaType === 'anime') {
                    navigate(`/anime/${item.itemId}`);
                  } else {
                    navigate(`/movie/details/${item.itemId}?type=${item.mediaType}`);
                  }
                }}
          ...
          <div className="text-center py-28 bg-white/5 rounded-2xl border border-dashed border-white/10 max-w-3xl mx-auto">
            ...
            <p className="text-gray-500 mt-1 max-w-sm mx-auto text-sm">Bookmark anime and movies to see them here!</p>
            <button 
              onClick={() => navigate('/')}
